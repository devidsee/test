...
class Planning:
    ...
    def get_min_max(self, poly, degrees):
        ...
        m, b = None, None
        m, b = line2d_angle((0, 0), degrees)
        dist_max = np.max([line2d_point_distance(m, b, (point[0], point[1])) for point in poly])
        dist_min = np.min([line2d_point_distance(m, b, (point[0], point[1])) for point in poly])
    ...
    def get_grated(self, slice, dist, one_dir=False, invert=False, degrees=0.0):
        ...
        dist_max = np.max([np.max([line2d_point_distance(m, b, (point[0], point[1])) for point in poly]) for poly in slice])
        dist_min = np.min([np.min([line2d_point_distance(m, b, (point[0], point[1])) for point in poly]) for poly in slice])
        ...
        for x in wide_range:
            points = []
            m_, b_ = parallel_line2d(m, b, x)
            for poly in slice:
                for k in range(len(poly)):
                    point = None
                    pnt1, pnt2 = poly[k - 1], poly[k]
                    d1 = line2d_point_distance(m_, b_, (pnt1[0], pnt1[1]))
                    d2 = line2d_point_distance(m_, b_, (pnt2[0], pnt2[1]))
                    if ((d1 > 0.0) != (d2 > 0.0)) or ((d1 == 0) != (d2 == 0)):
                        # Cortanse
                        my, by = line2d((pnt1[0], pnt1[1]),
                                             (pnt2[0], pnt2[1]))
                        intersec_x, intersec_y = line2d_intersec(m_, b_,
                                                                      my, by)
...
#                        contour = slice[1]
    ...
    def path_length(self, path):
        ...
        for k, (point, orient, process) in enumerate(path[:-1]):
            dist = distance(path[k][0], path[k+1][0])
            if process:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data', type=str,
                        default='../../data/piece7.stl',#/home/baltasar/Documentos/proper/points datapixel/P1DevMap/P1ModelDevMap.stl /home/baltasar/Documentos/celda4/proper/tacos gnc/taco.stl
    ...
    #    frame, t = quatpose_to_pose(position, orientation)
