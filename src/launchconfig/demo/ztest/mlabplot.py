...
class MPlot3D():
    ...
    def draw_transformation(self, matrix1, matrix2, label1='', label2=''):
        self.draw_frame(matrix_to_pose(matrix1), label=label1)
        self.draw_frame(matrix_to_pose(matrix2), label=label2)
        self.draw_line(matrix_to_pose(matrix1)[1],
                       matrix_to_pose(matrix2)[1])
...
if __name__ == '__main__':
    ...
    pose_arrow = rpypose_to_pose((0, 0, 100), (0, np.deg2rad(90), 0))
    trans_arrow = matrix_invert(pose_to_matrix(pose_arrow))
    print('arrow quat:', matrix_to_quatpose(trans_arrow))
    points = transform_points(pose_arrow, arrow)
    arrow2 = (points[0], points[1]-points[0])
    print(pose_arrow, points)

    position = ((0, 20, 0), (np.deg2rad(-45), np.deg2rad(45), np.deg2rad(0)))
    pose = rpypose_to_pose(*position)
    print('quat:', rpypose_to_quatpose(*position))
    trans = matrix_compose([pose_to_matrix(pose), pose_to_matrix(pose_arrow)])
    print('final pose:', matrix_to_quatpose(trans))
    points = transform_points(matrix_to_pose(trans), arrow)
    ...
    mplot3d.draw_frame(matrix_to_pose(trans_arrow))
    mplot3d.draw_arrow(arrow3)
    #mplot3d.draw_frame(matrix_to_pose(matrix_invert(trans)))
    mplot3d.show()
